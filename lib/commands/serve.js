...
class ServeCommand extends Command {
  async run () {
    ...
    const app = micro((req, res, ...args) => {
      ...
      if (routes.length) {
        const route = routes.find(({ src }) => req.url.match(src))
      ...
      const pathname = urls
        .map(url => `${url.replace(/^\//, '').replace(/\?.*/, '')}.js`.replace('.js.js', '.js'))
    ...
    app.listen(port, () => this.log(`Serving lambdas at http://localhost:${port}`))
