...
const lambdaSimulator = createApp => ({ entrypoint }) => async (req, res) => {
  ...
  const response = await agent[method](req.url)
    .set(req.headers)
    .send(body)
...
module.exports.simulators = {
  ...
  /**
   * @node/node and @node/node@canary builder simulator.
   */
  '@now/node': lambdaSimulator(lambda => lambdaHandler(lambda)),
  '@now/node@canary': lambdaSimulator(lambda => lambdaHandler(lambda)),
